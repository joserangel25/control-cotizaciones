...
export default function Cotizacion({ cotizacion }) {
  ...
  return (
    <li className='py-2 flex items-center border-b-2 justify-between'>
      ...
      {/* Botones para accion */}
      <div className='w-1/6 flex flex-col md:flex-row lg:w-1/12 items-center gap-2 justify-end'>
        <button title={estado !== 'Emitido' ? 'Editar' : 'No se puede modificar'} onClick={handleClickEditar} disabled={estado == 'Emitido'} >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className={`${estado === 'Emitido' ? 'text-stone-300' : 'hover:text-sky-800'} w-6 h-6 `}>
