...
export default function Cotizaciones() {
  ...
  const { totalOfertas, isLoading } = useTotal('Emitido');
  const { msg } = alerta; 
  return (
    <>
      <h2 className='font-black text-gray-700 text-4xl'>Emisiones</h2>
      { (!totalOfertas.length && !isLoading) && <Alerta alerta={{msg: 'No hay pólizas emitidas aún.'}} /> }
      {
